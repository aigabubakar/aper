...
<?= $this->section('title') ?>Academic — Employment History<?= $this->endSection() ?>
...
<div class="card mx-auto" style="max-width:1100px;">
  <div class="card-body">
    ...
    <form id="employmentForm" action="<?= site_url('profile/academic/employment/save') ?>" method="post" novalidate>
      <?= csrf_field() ?>
      ...
      <div class="row g-3 mb-3">
        ...
        <div class="col-md-3">
          <label class="form-label">CONUASS</label>
      ...
      <div class="d-flex justify-content-between">
        <a href="<?= site_url('profile/academic/personal') ?>" class="btn btn-outline-secondary">Back</a>
        <div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  form.addEventListener('submit', async function(e){
    ...
    try {
      ...
      else window.location.href = '<?= site_url('profile/academic/teaching_research') ?>';
...
<div class="mb-3">
  ...
  <div>
    <div class="form-check form-check-inline">
      <input class="form-check-input" type="radio" name="appointment_confirmed" id="apc_yes" value="yes" <?= (old('appointment_confirmed', $user['appointment_confirmed'] ?? '') == 'yes') ? 'checked' : '' ?>>
      <label class="form-check-label" for="apc_yes">Yes</label>
    </div>
    <div class="form-check form-check-inline">
      <input class="form-check-input" type="radio" name="appointment_confirmed" id="apc_no" value="no" <?= (old('appointment_confirmed', $user['appointment_confirmed'] ?? '') == 'no') ? 'checked' : '' ?>>
      <label class="form-check-label" for="apc_no">No</label>
    </div>
  </div>
</div>

<div id="appointmentConfirmedDateGroup" class="mb-3" style="display: none;">
  <label class="form-label">When?</label>
  <input type="date" name="appointment_confirmed_at" id="appointment_confirmed_at" class="form-control"
         value="<?= esc(old('appointment_confirmed_at', $user['appointment_confirmed_at'] ?? '')) ?>">
  <div class="form-text">If confirmed, please provide the confirmation date.</div>
</div>
